...
var Basic1 = function () {
    ...
    function computeCircle(midPoint, radius, segments) {
        ...
        for (var i = 1; i <= segments; i++) {
            ...
            var point = new Vector(midPoint.x + radius * Math.sin(angle), midPoint.z + radius * Math.cos(angle));   //Understand this. 
    ...
    function cullSegmentNormal(normal) {
        ...
        //              despite floating point imprecision.
			var dotVN = vec2.dot([0,1],[normal.x,normal.z]);
        	if (dotVN > -epsilon )
        	{
        	return true;
        	}
    ...
    function cullSegmentSimplified(lineSegment) {
        ...
        //              to the viewing direction are also culled
        //              despite floating point imprecision.
 		var lineDirection = lineSegment.endPoint.x - lineSegment.startPoint.x;
        if(lineDirection > -epsilon)
        {
        return true;
        }
        else
