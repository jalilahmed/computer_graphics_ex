var Basic2 = function () {
    ...
    function doAlphaBlending(index, images, alphas) {
		...
		if(images[i][index+3] != 0.0 )
		{
        var pixel_color = [images[i][index],images[i][index+1],images[i][index+2]];
		color =  vec3.add(color,vec3.scale(color,color,(1-alphas[i])),vec3.scale(pixel_color,pixel_color,alphas[i])); 
        }
        }
