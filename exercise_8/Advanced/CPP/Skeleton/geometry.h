...
struct Vertex {
	...
	Vertex(const vec3& p, const vec3& n, const vec2& t) :position(p), normal(n), texCoord(t) { };
...
public:
	...
	virtual void bindVertexArrayObjects(const vector<Vertex> &vertexdata, const vector<unsigned int> &indices)
	{
		...
		glEnableVertexArrayAttrib(vao,	// VAO name.
			2);							// Attribute index (layout in shader).

		glVertexArrayAttribFormat(vao,				// VAO name.
			2,										// Attribute index (layout in shader).
			2,										// Component count (x,y,z).
			GL_FLOAT,								// Type.
			GL_FALSE,								// Normalised.
			offsetof(Vertex, Vertex::texCoord));		// Offset (bytes).


		glVertexArrayVertexBuffer(vao,    // VAO name.
			2,										// Binding bindingindex.
			vbo,									// VBO name.
			offsetof(Vertex, Vertex::texCoord),		// Offset (bytes).
			sizeof(Vertex));						// Stride (bytes). THIS FUCKER IS SWITCHED WTF???
...
public:
	...
	{
		// inheritance does not make sense anymore
	};

	void createConfigurable(int slices = 44, int stacks = 22)
	{
		float r = 1;
		...
		vertexdata.reserve((slices+1)*(stacks+1));
		vector<unsigned int> indices;
		indices.reserve((slices + 1)*(stacks + 1)*6);
...
public:
	void create(string filename = "")
	{
		...
		vertexdata.push_back(Vertex(vec3(-1, -1, 0)));
		vertexdata.push_back(Vertex(vec3(1, -1, 0)));
		vertexdata.push_back(Vertex(vec3(1, 1, 0)));
		vertexdata.push_back(Vertex(vec3(-1, 1, 0)));
	...
	void create(string filename)
	{
		...
		for (int meshIndex = 0; meshIndex < scene->mNumMeshes; meshIndex++)
		{
			...
			for (uint32_t v = 0; v < mesh->mNumVertices; v++)
			{
				...
				vertices[v].texCoord = hasUV ? make_vec2(&mesh->mTextureCoords[0][v].x) : vec2(0.0f);
