...
void main() {
	...
	V =  - normalize(vec3(phongShaderConstData.V * phongShaderConstData.M * p));
	//
	...
	//
	//	TODO 8.1:
	//	Extrude Vertices conditionally
	//
	//	- compute Aateroid origin in worldspace
	//	- compute focal point
	//	- if vertex normal faces away from the sun, set position to focal point (do not forget transformation)
	//
	//	For Fragment Computation:
	...
	vec3 focal point = (pM[0],pM[1],pM[2] + 11.f);
	
	float dotNV = dot(N,V);
	
 	if(dotNV < 0){
 	gl_Position = phongShaderConstData.P * phongShaderConstData.V * phongShaderConstData.M * vec4(focalPoint,1.f);
 	shadowDistance = 1;
 	}
 	else{
 	shadowDistance = 0;
