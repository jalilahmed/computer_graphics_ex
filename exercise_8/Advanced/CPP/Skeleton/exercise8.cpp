...
class SceneNode
{
	public:
		...
		virtual void draw(bool renderTransparent)
		{ 
...
class InstancedSceneNode : public  SceneNode
{
	public:
		...
		{
			if(phongShaderInstancedTransformationDataBuffer == 0)
		...
		void draw(bool renderTransparent)
		{ 
			if(shader != 0)
			{
			 	phongShaderConstData.M = M * glm::scale(scale, scale, scale);;
				...
				//cout << phongShaderInstancedTransformationDataBuffer << endl; 
			}
...
void initGL() {
	//glDepthMask(GL_TRUE);
	glClearDepth(1);
	glClearColor(3.f/255, 3.f / 255, 3.f / 255, 0);   // set the clear color to black
	...
	createProgram_VGF("clouds", "shader/clouds_VS.glsl", "shader/clouds_GS.glsl", "shader/clouds_FS.glsl", &cloudShader, true);
	...
	//
	 ...
	 createProgram_VF("asteroidShadowInstanced", "shader/asteroidShadowInstanced_VS.glsl", "shader/asteroidShadow_FS.glsl", &asteroidShadowShader, true);
...
int  main(int argc, char **argv)
{
	...
	cam->setP(perspective(70.0f, (GLfloat)windowSize.x / (GLfloat)windowSize.y, .50f, 4000.f));
	...
	root->attachChild(asteroidBelt);
	 ...
	 InstancedSceneNode* asteroidBeltShadow = new InstancedSceneNode(asteroidMesh, vec3(0, 0, 0));
	 asteroidBeltShadow->animate = [](float deltaT, glm::vec3& position, glm::quat& rotation)
	 {
	 	float angle = .015f / M_PI * deltaT;
 	 ...
 	 asteroidBeltShadow->material = new Material(vec3(.7, .7, .7));
 	 asteroidBeltShadow->shader = asteroidShadowShader;
	  
	 asteroidBeltShadow->isTransparent = true;
	 
	 asteroidBeltShadow->instancecount = asteroidBelt->instancecount ;
	 root->attachChild(asteroidBeltShadow);
	...
	while (running)
	{
		...
		sunShaderConstData.t += deltaT.count();
		 ...
		 scene.draw(false); //renders opaque
		 ...
		 //
		 //	TODO 8.1:
		 //	Implement blending of transparent geometry
		 //
		 //////////////////////////////////////////////////////////////////////////////
		glDepthMask(GL_FALSE); //Why the result is the same even if I turn off Depth Write or not?
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);	
		// Question Here. Should we disable it or not. because if I dont disable then the asteroids are rendered on the earth. 
		//glDisable(GL_CULL_FACE);
		scene.draw(true);
		glDisable(GL_BLEND);  // If I donnot disable the gl_blend then the earth cloud shadow show texture. 
		glDepthMask(GL_TRUE);
