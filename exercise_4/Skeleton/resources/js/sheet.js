...
function setupTasks(scope, number) {
    ...
    for (var i in elements){
        ...
        if (task.nodeType == Node.ELEMENT_NODE) {
          ...
          h2.innerHTML = prefix + " " + number + "." + partNr + " <strong>" + title + "</strong>" + " [" + pointsTask +" points]" ;
		  if (task.getAttribute("submitfile")) {
		      h2.innerHTML = h2.innerHTML + "<br><p><span style='color:black'>- Submission file(s): </span><i><span style='color:red'>" + submitFile + "</span></i></p>";
		  }
...
function setupSubTasks(scope, number, taskNr, pointsTask) {
    ...
    for (var i in elements){
        ...
        if (subtask.nodeType == Node.ELEMENT_NODE) {
		  ...
		  var submitFile = "<span style='color:magenta'>[please add a submit file attribute]</span>";
          if (subtask.getAttribute("submitfile")) {
              submitFile = subtask.getAttribute("submitfile")
          }
		  
          var h3 = document.createElement("h3");
          h3.innerHTML = "<span class='enum'>" + String.fromCharCode(97 + partNr) + ")</span> <strong>" + title + "</strong>"  + " [" + pointsSubTask +"/"+ pointsTask + "]";
          if (subtask.getAttribute("submitfile")) {
			h3.innerHTML =  h3.innerHTML + "<br><p>- Submission file(s): <i><span style='color:red'>" + submitFile + "</span></i></p>";
		  }
  ...
  for (var i in elements){
    ...
    if (node.getAttribute) {
      ...
      loadResource(url, node, function(text, payload) {
        ...
        if (payload.getAttribute("data-inner-onload")) {
            eval(payload.getAttribute("data-inner-onload"));
        }
      })
...
function didLoadShader(id, scriptNodeId){
    ...
    if (scriptNodeId) {
        var shader = document.querySelector("#"+id);
        //console.log(shader)
...
function didLoadCall(script) {
    ...
    if (script && script.getAttribute('data-call-depends')) {
        var depends = JSON.parse(script.getAttribute('data-call-depends'))
        var onload = script.getAttribute('data-call-onload')
        var d = depends.length;
        
        while (d--) {
            var name = depends[d];    
            var g = GLOBAL_SHADERS.length;
            var one = false;
            while (g--) {
                var gname = GLOBAL_SHADERS[g]
                if (gname==name) {
                    one = true;
                    break;
                }
...
function callScripts(scope) {
  ...
  for (var i in elements){
    ...
    if (node.getAttribute) {
      ...
      var shaders = node.querySelectorAll('shader');
      var dependencies = [];
      for (var i in shaders){
        var shader = shaders[i]

        if (shader.getAttribute) {
            var type = shader.getAttribute('type');
      ...
      list.push({name:methodName, element:node, id:callId, waitImage:img, depends:dependencies})
      callId++
    }
  }
...
function executeCalls(files, container){
  ...
  for (var file in files) {
    ...
    for (var i in data) {
      ...
      depends = depends.concat(info.depends);
    }
    //console.log(file, calls)
    var script = document.createElement("script");
    script.setAttribute("type", "text/javascript")
    script.setAttribute("src", file+"?t="+Math.random())
    script.setAttribute("async", "")
    script.setAttribute("data-call-onload", calls)
    script.setAttribute("data-call-depends", JSON.stringify(depends));
    script.setAttribute("onload", "didLoadCall(this)" )
